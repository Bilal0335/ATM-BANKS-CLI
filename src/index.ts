...
let account_balance: number = 2000;
const account_number: string | number = 12345;
...
async function performTransaction() {
  ...
  if (transType.Transaction_type === "Balance Inquiry") {
    console.log(
      chalk.green(`Here is Your Account Balance: $${account_balance}`)
    );
    ...
    if (account_balance >= withdrawAmount && withdrawAmount > 0) {
      ...
      console.log(
        chalk.green(`Your remaining account balance: $${account_balance}`)
      );
    } else {
